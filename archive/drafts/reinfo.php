...
<div class="padding" id="asd">
    ...
    <p class="transition visible">
        ...
        <div class="ui existing segment">
          <pre v-if="escape" style="margin-top: 0; margin-bottom: 0; "><code><div id='errexp'><?php
		    if($r_result>=0 && $r_result<4)echo '아직 채점이 완료되지 않은 제출입니다.';
		    if($r_result==4)echo '모든 테스트 케이스에서 제한 시간 내에 정답을 출력했습니다.';
		    else if($r_result==5)echo'출력된 형식이 문제가 요구하는 출력 형식과 다릅니다.<br>문제의 출력 형식과 공백, 줄바꿈 등을 다시 확인해 보세요.';
		    else if($r_result==6)echo'틀린 답을 출력했습니다.<br>문제의 조건을 다시 확인해 보고, 논리적 오류나 코드 상의 오류, 코너 케이스 등을 고려해 보세요.';
		    else if($r_result==7)echo'제한 시간 내에 결과를 출력하지 못했습니다.<br>문제의 입력 제한과 코드의 시간 복잡도 등을 다시 확인해 보세요.';
		    else if($r_result==8)echo'문제의 메모리 제한을 초과했습니다.<br>너무 많은 양의 메모리를 할당하지는 않았는지, 스택 오버플로우가 일어나지는 않는지 다시 확인해 보세요.';
		    else if($r_result==9)echo'문제가 요구하는 출력보다 많은 양의 출력이 이뤄졌습니다.<br>출력을 진행하는 코드 부분을 다시 확인해 보세요.';
	   	    else if($r_result==14)echo'수동 확인 대기중인 문제입니다.';
		?></div></code></pre>
        </div>
    </div>
</div>
<script>
    ...
    function explain(){
	if(<?php echo $r_result;?>!='10')return;
      var errmsg = <?php echo json_encode($view_reinfo); ?>;
      var expmsg = "";
