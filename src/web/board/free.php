...
<div class="board_wrap"></div>
		...
		$offset = ($page - 1) * 18;
		$sql = "SELECT `post_id`, `problem_id`, `title`, `writer`, `created_time` FROM `post` Where category='자유' ORDER BY `post_id` DESC LIMIT $offset, 18";
		$result = pdo_query($sql);
		foreach ($result as $row) {
		...
		$sql = "SELECT count(post_id) FROM post Where category='자유'";
		$result = pdo_query($sql);
		...
		?>
            ...
            <div class="board_page">
                <a href="board.php?data=free" class="bt first"><<</a>
                <a href="board.php?page=<?php echo $page - 1; ?>&data=free" class="bt prev"><</a>		
                <a href="board.php?page=<?php echo $next_page; ?>&data=free" class="bt next">></a>
                <a href="board.php?page=<?php echo $last_page; ?>&data=free" class="bt last">>></a>
            </div>
