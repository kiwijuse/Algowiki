...
<div class="board_wrap"></div>
		...
		$offset = ($page - 1) * 18;
		$sql = "SELECT `post_id`, `problem_id`, `title`, `writer`, `created_time` FROM `post` ORDER BY `post_id` DESC LIMIT $offset, 18";
		$result = pdo_query($sql);
		foreach ($result as $row) {
	    ...
	    <div class="board_page">
                <a href="board.php" class="bt first"><<</a>
                <a href="board.php?page=<?php echo $page - 1; ?>" class="bt prev"><</a>		
                <a href="board.php?page=<?php echo $next_page; ?>" class="bt next">></a>
                <a href="board.php?page=<?php echo $last_page; ?>" class="bt last">>></a>
            </div>
