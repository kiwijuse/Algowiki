...
$sql = "SELECT `problem_id`,`title`,`content`,`category`,`writer` FROM `post` where post_id = '$post_id' ";
$result = pdo_query($sql);
$problem_id = $result[0][0];
$title = $result[0][1];
$content = $result[0][2];
$category = $result[0][3];
$writer=$result[0][4];
...
<body class="bg-gray-100">
    <h1 class="text-xl mb-4 font-semibold text-gray-700">게시글 수정</h1>
	<form method=POST action=post_edit_commit.php?id=<?php echo $post_id; ?>>
	<input type="hidden" name="writer" value="<?php echo $writer; ?>" class="hidden">
    <div class="mb-4">
	
      <label class="block text-gray-700 text-sm font-bold mb-2" for="post-title">
        제목
      </label>
      <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $title; ?>" id="post-title" name="title" type="text" placeholder="제목 입력">
    </div>
    <div class="mb-4 flex">
      <div class="relative w-1/4 mr-4">
        ...
        <input readonly class="shadow appearance-none border rounded width:30px py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $category; ?>" id="classification" name="category" type="text" placeholder="분류" value="자유" onfocus="showDropdown()" onblur="hideDropdown()">
...
<div class="relative w-1/2 mr-4">
        ...
        <input class="shadow appearance-none border rounded min-width:30px; py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="<?php echo $problem_id; ?>" name="problem_id" type="text" placeholder="기입 안해도 상관 X">
      </div>
</div>
    <div class="mb-6">
      ...
      <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"  id="content" name="content" placeholder="내용 입력" rows="30"><?php echo $content; ?></textarea>
    </div>
